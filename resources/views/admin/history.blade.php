...
<head>
    ...
    <title>{{ __('welcome.تاريخنا') }}</title>
</head>
        ...
        <div id="page-content-wrapper">
            ...
            <div class="h-500px scroll" id="page-content-wrapper">
                <div class="container  p-24">
                    <div class="row g-3 justify-content-between  m--100">
                        <h5 class=" fs-20 text_cap">{{ __('welcome.عن الشركة') }}</h5>
                        <div class="">
                            <div class="form-check form-switch m-0">
                                <input class="form-check-input status-checkbox" type="checkbox"
                                role="switch" id="{{ $data->id }}"
                                data-partner-id="{{ $data->id }}"
                                {{ $data->status ? 'checked' : '' }}>
                            </div>
                        </div>
                        <form action="{{ url('/updateHistory') }}" method="post">
                            <input type="hidden" name="id" value="{{ $data->id }}">
                            @csrf
                        <div class="col-lg-12">
                            <h5 class="fs-18 text_cap">{{ __('welcome.تاريخنا') }}</h5>
                            ...
                            <div class="d-flex flex-column gap-4c my-3">
                                <label class="fs-12 fw-400-20px text-1 text_cap">{{ __('welcome.الوصف') }}</label>
